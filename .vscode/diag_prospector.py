...
async def main():
    ...
    print("\n" + "=" * 70)
    print("TEST 3: FULL hunt+scrub pipeline — 5 clinical labs in NC")
    print("=" * 70)
    result = await prospect_and_scrub("NC", specialty="clinical", limit=5)
    print(f"summary: {result['summary']}")
    print(f"source:  {result.get('prospect_source')}")
    print(f"\n🔥 Top leads:")
    for r in result.get("daily_top_10", [])[:5]:
