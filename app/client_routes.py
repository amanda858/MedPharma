...
@router.post("/admin/diag/restore-claims-from-backup")
def admin_diag_restore_claims_from_backup(
    file: str,
    from_client_id: int,
    to_client_id: int,
    hub_session: Optional[str] = Cookie(None),
):
    """Admin-only: surgically restore claims_master + payments from a backup DB into
    the LIVE db, remapping the account id (an account delete cascaded its claims, so
    the rows must land on the account's NEW id).

    Safe by design:
      * Reads the backup via ATTACH and only SELECTs from it — the backup is untouched.
      * Copies only rows WHERE client_id = from_client_id, rewriting client_id to
        to_client_id. Every other column (incl. id + uploaded_by attribution) is copied
        verbatim, so per-user tallies and payment↔claim links survive.
      * Column-intersection: copies only columns present in BOTH schemas.
      * Idempotent: clears the destination account's rows first, so re-running yields
        the same result instead of duplicating.
    """
    _require_full_admin(hub_session)
    from .config import DATABASE_PATH
    ...
    def _intersect_cols(conn, table: str) -> list:
        live_cols = [r[1] for r in conn.execute(f"PRAGMA table_info({table})").fetchall()]
        bk_cols = {r[1] for r in conn.execute(f"PRAGMA bk.table_info({table})").fetchall()}
        return [c for c in live_cols if c in bk_cols]

    def _copy_table(conn, table: str) -> dict:
        cols = _intersect_cols(conn, table)
        if "client_id" not in cols:
            return {"table": table, "skipped": "no client_id column"}
        insert_list = ", ".join(cols)
        select_list = ", ".join(("? AS client_id" if c == "client_id" else c) for c in cols)
        before = conn.execute(
            f"SELECT COUNT(*) FROM {table} WHERE client_id=?", (to_client_id,)).fetchone()[0]
        conn.execute(f"DELETE FROM {table} WHERE client_id=?", (to_client_id,))
        conn.execute(
            f"INSERT INTO {table} ({insert_list}) "
            f"SELECT {select_list} FROM bk.{table} WHERE client_id=?",
            (to_client_id, from_client_id),
        )
        after = conn.execute(
            f"SELECT COUNT(*) FROM {table} WHERE client_id=?", (to_client_id,)).fetchone()[0]
        return {"table": table, "before": before, "restored": after}

    conn = get_db()
    try:
        conn.execute("ATTACH DATABASE ? AS bk", (src,))
        try:
            claims_before = conn.execute("SELECT COUNT(*) FROM claims_master").fetchone()[0]
            results = [_copy_table(conn, "claims_master")]
            try:
                results.append(_copy_table(conn, "payments"))
            except Exception as e:
                results.append({"table": "payments", "error": f"{type(e).__name__}: {e}"})
            conn.commit()
            claims_after = conn.execute("SELECT COUNT(*) FROM claims_master").fetchone()[0]
            charged = conn.execute(
                "SELECT COALESCE(SUM(ChargeAmount),0) FROM claims_master WHERE client_id=?",
                (to_client_id,)).fetchone()[0]
        finally:
    ...
    return {
        ...
        "claims_total_before": claims_before,
        "claims_total_after": claims_after,
        "restored_charged": round(float(charged or 0), 2),
        "tables": results,
    }
